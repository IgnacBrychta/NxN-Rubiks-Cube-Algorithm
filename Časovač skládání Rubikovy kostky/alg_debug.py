...
suppressed = False
###################################### TO DO: #########################################x
# ~~fix saving system (custom categories), Aos, DNF
...
def visualize(input_scramble,available_turns):
	...
	def find_a_solution():
		...
		global cube_nxn,cube_nxn_solved
		solution=[]
		# Ze souřadnic vrátí barvy rohu
		def get_corner_colors(corner):
			global cube_nxn
			corner=eval(corner)
			print("function get_corner_colors")
			print(corner)
			cord1=corner[0]
			cord2=corner[1]
			cord3=corner[2]
		...
		def add_move_to_solution(move):
			global algorithm_solution,cube_nxn
			algorithm_solution.append(move)
			do_move(move,show=False)
		# Srovnání kostky - w nahoře, r vepředu
		while cube_nxn[0][4]!=c and cube_nxn[2][4]!=c and cube_nxn[5][4]!=c:
			print("case1")
			add_move_to_solution("y")
		while cube_nxn[0][4]!=c:
			add_move_to_solution("x")
		while cube_nxn[2][4]!=r:
			print("case2")
			add_move_to_solution("y")
		...
		def cross():
			...
			for _ in range(8):
				...
				print(f"checking this edge: {edge} | {bottom_edge}")
				if y in bottom_edge:
					print(f"yellow found in a bottom edge | {bottom_edge}")
			...
			for _ in range(4):
				...
				print(f"checking this edge: {edge} | {side_edge}")
				if y in side_edge:
			...
			for _ in range(4):
				...
				print(f"checking this edge: {edge} | {top_edge}")
				if y in top_edge:
		...
		def corners_f():
			global cube_nxn
			print("-------------------------- CORNERS ----------------------------------------------------------------------------------------------------")
			#while cube_nxn[2][4] not in corner_pos_1 and cube_nxn[3][4] not in corner_pos_1 and y not in corner_pos_1:
			print("SOLUTION FOR THE CROSS AND THE POSITION OF THE CORNERS:")
			print(algorithm_solution)
			for corner_pos_1 in [[r,b,y],[b,o,y],[g,o,y],[g,r,y]]:
				corner_found=False
				print("this corner is now being worked on solving:")
				print(corner_pos_1)
				print(f"{cube_nxn[2][4]} is in the front")
				print("case1")
				if set(get_corner_colors(str([[5,2],[3,6],[2,8]])))!=set(corner_pos_1): # if not in the right spot
					print("case2")
					for _ in range(4):
						corner_pos_2=set(get_corner_colors(str([[0,6],[1,2],[2,0]])))
						if set(corner_pos_2)==set(corner_pos_1):
							print("case3")
							add_move_to_solution("3L'")
							add_move_to_solution("U'")
							add_move_to_solution("3L")
							...
							print("case4")
							add_move_to_solution("U'")
					print("corner is not in the top layer, trying the bottom layer")
					if not corner_found:
						for _ in range(4):
							add_move_to_solution("3U'")
							corner_pos_2=set(get_corner_colors(str([[5,2],[3,6],[2,8]])))
							if set(corner_pos_2)==set(corner_pos_1):
								print("matching corner has been found:")
								print(set(corner_pos_1))
								add_move_to_solution("3L'")
								add_move_to_solution("U")
								add_move_to_solution("3L")
								limit=0
								while cube_nxn[2][7]!=cube_nxn[2][4]:
									...
									if limit>8:
										...
										print("scramble:")
										print(scramble)
										raise Exception("something went wrong with inserting the right corner, idk what")
					...
					print("correct corner already in place")
				add_move_to_solution("y")
			print(cube_nxn)
			corner=[[5,2],[3,6],[2,8]]
			for _ in range(4):
				...
				print("--- corner info ---")
				print(f"corner: {corner} | corner colors: {corner_colors}")
				for _ in range(6):
				    if corner_colors[0]!=y:
				        print("corner not rotated right")
				        print("--- corner info ---")
				        print(f"corner: {corner} | corner colors: {corner_colors}")
				        for move in ["3L'","U","3L","U'","3L'","U","3L","U'"]:
				        	add_move_to_solution(move)
				        corner_colors = get_corner_colors(str(corner))
				        print(f"corner after rotation: {corner_colors}")
				    else:
				    	print(f"corner rotated right: {corner_colors}")
				    	break
				add_move_to_solution("3U'")
		def second_layer():
			global cube_nxn
			...
			print("SOLUTION FOR THE CROSS AND CORNERS:")
			print(algorithm_solution)
			for _ in range(8): # Second layer - leave solved pieces in place or take unsolved ones out
				print(f"Edge in slot: {edge}")
				if cube_nxn[2][4]==edge[0] and cube_nxn[3][4]==edge[1]: # if the right edge is in the right spot, do nothing
					#pass
					print("case1 - edge is solved")
				elif cube_nxn[2][4]==edge[1] and cube_nxn[3][4]==edge[0]:
					for move in ["3L'","U","3L","U'","3L'","U","3L","U","F'","U'","F","U","F'","U'","F"]: # if the right edge is in the right spot but flipped, flip it
							add_move_to_solution(move)

					print("case2 - edge was flipped, is now solved")
				else:
					print("case3 - edge not in the right spot, trying to find it in the U layer")
					# if the correct piece is present in the top layer, insert it
					limit=0
					for _ in range(4):
						print("top layer edge ([0][3] | [1][1]):")
						print(set([cube_nxn[0][3],cube_nxn[1][1]]))
						print("slot:")
						print(set([cube_nxn[2][4],cube_nxn[3][4]]))
						if set([cube_nxn[0][3],cube_nxn[1][1]])!=set([cube_nxn[2][4],cube_nxn[3][4]]):
							add_move_to_solution("U'")
					if [cube_nxn[0][3],cube_nxn[1][1]]==[cube_nxn[3][4],cube_nxn[2][4]]: # Insert the right piece into the right slot the right way
						moves=["3L'","U","3L","U'","F'","U'","F"]
						print("case5 - edge found, insert edge from [0][3]")
					elif [cube_nxn[0][3],cube_nxn[1][1]]==[cube_nxn[2][4],cube_nxn[3][4]]:
						moves=["U","F'","U'","F","U","3L'","U","3L"]
						print("case6 - edge found, insert edge from [0][1]")
					else:
						print("case7 - edge not found, inserting a 'random' edge")
		...
		def move_cross_on_top():
			...
			#for i in range(4):
				#correct_edge_order.append([order[i:]+order[:i]])
			for _ in range(4):
				...
				for _ in range(4):
					print(f"iteration: {_}")
					print([cube_nxn[3][4],cube_nxn[4][4]])
		...
		def move_corners():
			...
			print(cube_nxn)
			for corner in [[[0,8],[2,2],[3,0]],[[0,6],[1,2],[2,0]],[[0,0],[4,2],[1,0]],[[0,2],[3,2],[4,0]],[[5,0],[2,6],[1,8]],[[5,2],[3,6],[2,8]],[[5,6],[1,6],[4,8]],[[5,8],[4,6],[3,8]]]:
				print(get_corner_colors(str(corner)))
			...
			for _ in range(6):
				...
				print("correct edge:")
				print(set([w,cube_nxn[2][1],cube_nxn[3][1]]))
				# if the corner in the UFR is not equal to the edge that's supposed to be in the UFR slot
				if y in set(corner_base):
					print("skipping")
				elif set(corner_base)!=set([w,cube_nxn[2][1],cube_nxn[3][1]]):
					print("incorrect corner inserted")
					add_move_to_solution("3L")
					add_move_to_solution("3U")
					add_move_to_solution("3L'")
					limit=1
		...
		def rotate_top_corners():
			#return
			print("------------------------------------- ROTATE TOP CORNERS ---------------------")
			for _ in range(4):
				corner=get_corner_colors(str([[0,8],[2,2],[3,0]]))
				print(corner)
				for _ in range(3):
					if cube_nxn[0][8]!=w:
						print("white not on top")
		...
		def simplify_solution():
			...
			simplified_solution=[]
			for x in range(len(algorithm_solution)):
				...
				try:
					...
					simplified_solution.append([item1])
			#return
			for x in range(len(simplified_solution)):
				if len(simplified_solution[x])>=4:
					simplified_solution[x]=simplified_solution[x][4:]
				if len(simplified_solution[x])>=2:
					#simplified_solution[x]=[simplified_solution[x][y]+" "*(3-y) for y in range(len(simplified_solution[x]))]
					if len(simplified_solution[x])==3:
						simplified_solution[x]=[simplified_solution[x][0][0:2]]
					elif len(simplified_solution[x])==2:
						simplified_solution[x]=[simplified_solution[x][0][0:2]+"2"]
					else:
						simplified_solution[x]=[simplified_solution[x][0][0:2]+"'"]

			simplified_length=len(sum(simplified_solution,[]))
	...
	def debug_moves():
		for move in ['x', 'y', 'y', 'y', "3U'", "3U'", 'U', 'F', 'U', "L'", "3U'", "3U'", "3U'", "3U'", 'F', 'U', "L'", "3U'", 'F', 'U', 'U', "L'", "3U'", "2U'", "2U'", 'U', 'U', "F'", "2U'", "2U'", "U'", "U'", "U'", "U'", 'F2', "U'", "U'", '3L2', "U'", "U'", '3F2', "U'", "U'", 'L2', "3L'", "U'", '3L', 'y', "U'", "U'", "U'", "U'", "3U'", "3U'", "3L'", 'U', '3L', '3U', '3U', "3L'", "U'", '3L', 'y', "U'", "U'", "U'", "U'", "3U'", "3U'", "3U'", "3L'", 'U', '3L', '3U', '3U', '3U', "3L'", "U'", '3L', 'y', "U'", "3L'", "U'", '3L', 'y', "3L'", 'U', '3L', "U'", "3L'", 'U', '3L', "U'", "3U'", "3L'", 'U', '3L', "U'", "3L'", 'U', '3L', "U'", "3U'", "3U'", "3L'", 'U', '3L', "U'", "3L'", 'U', '3L', "U'", "3L'", 'U', '3L', "U'", "3L'", 'U', '3L', "U'", "3U'"]:
			do_move(move,show=False)
		show_cube()
	# Panel tlačítek
	def buttons_gui(): 
